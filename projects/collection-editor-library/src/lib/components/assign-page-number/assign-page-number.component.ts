...
export class AssignPageNumberComponent implements OnInit {
  ...
  treeEventListener(event) {
    ...
    const hierarchy = this.editorService.getHierarchyObj(
      data,
      "",
      event?.identifier
    );
    this.questionService
      .getQuestionList(
        _.get(hierarchy[event?.identifier], "children"),
        "editorState"
      )
      .subscribe(
        (response: any) => {
    ...
    this.pageNumArray.forEach((entry, index) => {
      if (!_.isEmpty(entry)) {
        entry.forEach((identifier) => {
          const getQuestion = this.questions.find(
  ...
  updateRenderingSequence() {
    ...
    if (this.createArray.length === this.pageNumArray.length) {
      ...
      newArray = result.splice.apply(
        result,
        [0, this.createArray.length].concat(this.createArray)
      );
    } else {
      newArray = [
        ...this.pageNumArray,
        ...this.createArray.slice(this.pageNumArray.length),
      ];
    }
  ...
  createSequence(data) {
    this.recordedBy = _.get(
      this.treeService.getFirstChild(),
      "data.metadata.recordedBy"
    );
    // tslint:disable-next-line:variable-name
    const dataArray = [];
    // tslint:disable-next-line:variable-name
