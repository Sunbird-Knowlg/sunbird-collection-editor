...
export class MetaFormComponent implements OnChanges, OnDestroy {
  ...
  setAppIconData() {
    ...
    this.appIconConfig = _.find(
      _.flatten(_.map(this.rootFormConfig, "fields")),
      { code: "appIcon" }
    );
  ...
  showShuffleMessage(event) {
    this.subscription = this.helperService.shuffleValue.subscribe(
      (shuffle) => (this.previousShuffleValue = shuffle)
    );
    if (
      _.isBoolean(event.shuffle) &&
      event.shuffle === true &&
      _.isBoolean(this.previousShuffleValue) &&
      this.previousShuffleValue === false
    ) {
      this.toasterService.simpleInfo(
        _.get(this.configService, "labelConfig.lbl.shuffleOnMessage")
      );
    }
      ...
      this.frameworkService.frameworkData$
        .pipe(
          ...
          filter((data) =>
            _.get(
              data,
              `frameworkdata.${this.frameworkService.organisationFramework}`
            )
          )
        )
          ...
          if (frameworkDetails && !frameworkDetails.err) {
            const frameworkData =
              frameworkDetails.frameworkdata[
                this.frameworkService.organisationFramework
              ].categories;
            this.frameworkDetails.frameworkData = frameworkData;
            this.frameworkDetails.topicList = _.get(
              _.find(frameworkData, {
                code: "topic",
              }),
              "terms"
            );
            this.frameworkDetails.targetFrameworks = _.filter(
              frameworkDetails.frameworkdata,
              (value, key) => {
                return _.includes(
                  this.frameworkService.targetFrameworkIds,
                  key
                );
              }
            );
  ...
  attachDefaultValues() {
    ...
    const categoryMasterList =
      this.frameworkDetails.frameworkData ||
      (!isRootNode &&
        this.frameworkService.selectedOrganisationFramework &&
        _.get(
          this.frameworkService.selectedOrganisationFramework,
    ...
    let formConfig: any =
      _.get(metaDataFields, "visibility") === "Default" || isRootNode
        ? _.cloneDeep(this.rootFormConfig)
        : _.cloneDeep(this.unitFormConfig);
    formConfig =
      formConfig && _.has(_.first(formConfig), "fields")
        ? formConfig
        : [{ name: "", fields: formConfig }];
      ...
      _.forEach(this.frameworkDetails.targetFrameworks, (framework) => {
        _.forEach(formConfig, (section) => {
          _.forEach(section.fields, (field) => {
            const frameworkCategory = _.find(
              framework.categories,
              (category) => {
                return (
                  category.code === field.sourceCategory &&
                  _.includes(field.code, "target")
                );
              }
            );
    ...
    _.forEach(formConfig, (section) => {
      _.forEach(section.fields, (field) => {
          ...
          if (_.has(metaDataFields, field.code)) {
            ...
            field.default = !_.isEmpty(value)
              ? moment
                  ...
                  .format(this.helperService.getTimerFormat(field))
              : "";
          }
        }
        ...
        if (
          !_.includes(field.depends, "framework") &&
          !_.includes(field.code, "target")
        ) {
          const frameworkCategory = _.find(categoryMasterList, (category) => {
            return (
              category.code === field.sourceCategory ||
              category.code === field.code
            );
          });
          if (!_.isEmpty(frameworkCategory)) {
            field.terms = frameworkCategory.terms;
        ...
        if (field.code === "license") {
          ...
          field.default = !_.isEmpty(defaultLicense)
            ? defaultLicense
            : this.editorService.editorConfig.context.defaultLicense;
        ...
        if (field.code === "additionalCategories") {
          ...
          const additionalCategories = _.uniq(
            _.get(
              channelInfo,
              `${
                this.configService.categoryConfig.additionalCategories[
                  this.editorService.editorConfig.config.objectType
                ]
              }`
            ) ||
              _.get(
                this.editorService.editorConfig,
                "context.additionalCategories"
              )
          );
        ...
        if (field.code === "copyright") {
          ...
          if (
            ...
            this.editorService.editorConfig.config.setDefaultCopyRight
          ) {
        ...
        if (field.code === "maxQuestions") {
          ...
          const rootFirstChildNode =
            this.editorService.getContentChildrens(activeNode);
          if (rootFirstChildNode && rootFirstChildNode.length > 0) {
            field.range = _.times(
              _.size(rootFirstChildNode),
              (index) => index + 1
            );
          }
        }
        ...
        if (field.code === "author") {
          ...
          field.default = !_.isEmpty(defaultAuthor)
            ? defaultAuthor
            : _.get(this.editorService.editorConfig, "context.user.fullName");
        }
        ...
        if (field.code === "allowECM") {
          field.default =
            _.get(metaDataFields, "recordedBy") !== "Self" ? "Yes" : "No";
        }

        if (field.code === "instances") {
          field.default = !_.isEmpty(metaDataFields, "instances")
            ? _.get(metaDataFields, "instances.label")
            : "";
        }

        if (
          (_.isEmpty(field.range) || _.isEmpty(field.terms)) &&
          !field.editable &&
          !_.isEmpty(field.default)
        ) {
  ...
  isReviewMode() {
    return _.includes(
      ["review", "read", "sourcingreview", "orgreview"],
      this.editorService.editorMode
    );
  }
  ifFieldIsEditable(fieldCode, primaryCategoryEditableConfig?) {
    ...
    const editableFields = _.get(
      this.editorService.editorConfig.config,
      "editableFields"
    );
    if (
      editableFields &&
      !_.isEmpty(editableFields[ediorMode]) &&
      _.includes(editableFields[ediorMode], fieldCode)
    ) {
      return true;
    }
    return false;
  ...
  showTimer(
    ...
    const response = merge(
      ..._.map(depends, (depend) => depend.valueChanges)
    ).pipe(
      switchMap((value: any) => {
        const isDependsInvalid = _.includes(
          _.map(depends, (depend) => depend.invalid),
          true
        );
  ...
  getFramework(
    ...
    const response = control.valueChanges.pipe(
      switchMap((value: any) => {
        if (!_.isEmpty(value)) {
          return framworkServiceTemp.getFrameworkCategories(value).pipe(
            map((res) => {
              const frameworkResponse = _.get(res, "result");
              framworkServiceTemp.selectedOrganisationFramework =
                frameworkResponse;
