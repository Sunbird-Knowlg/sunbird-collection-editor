...
export class QuestionComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.initialize();
    this.framework = _.get(
      this.editorService.editorConfig,
      "context.framework"
    );
  ...
  fetchFrameWorkDetails() {
    this.frameworkService.frameworkData$
      .pipe(
        ...
        filter((data) => _.get(data, `frameworkdata.${this.framework}`)),
        take(1)
      )
      .subscribe((frameworkDetails: any) => {
        if (frameworkDetails && !frameworkDetails.err) {
          const frameworkData =
            frameworkDetails.frameworkdata[this.framework].categories;
          this.frameworkDetails.frameworkData = frameworkData;
          this.frameworkDetails.topicList = _.get(
            _.find(frameworkData, { code: "topic" }),
            "terms"
          );
  ...
  initialize() {
    this.editorService.fetchCollectionHierarchy(this.questionSetId).subscribe(
      (response) => {
        this.questionSetHierarchy = _.get(response, "result.questionSet");
        const parentId = this.editorService.parentIdentifier
        ...
        if (
          ...
          this.sectionPrimaryCategory = _.get(
            response,
            "result.questionSet.primaryCategory"
          );
          this.selectedSectionId = _.get(sectionData, "data.metadata.parent");
          this.getBranchingLogic(children);
        }
        this.questionFormConfig = _.cloneDeep(this.leafFormConfig);
        let leafFormConfigFields = _.join(
          _.map(this.leafFormConfig, (value) => value.code),
          ","
        );
        leafFormConfigFields += ",isReviewModificationAllowed";
        if (!_.isUndefined(this.questionId)) {
          this.questionService
            .readQuestion(this.questionId, leafFormConfigFields)
            .subscribe(
              (res) => {
                if (_.get(res, "result")) {
                  this.questionMetaData = _.get(res, "result.question");
                  this.questionPrimaryCategory = _.get(
                    this.questionMetaData,
                    "primaryCategory"
                  );
                  // tslint:disable-next-line:max-line-length
                  this.questionInteractionType = _.get(
                    this.questionMetaData,
                    "interactionTypes"
                  )
                    ? _.get(this.questionMetaData, "interactionTypes[0]")
                    : "default";
                  this.editorService.setIsReviewModificationAllowed(
                    _.get(
                      this.questionMetaData,
                  ...
                  if (this.questionInteractionType === "slider") {
                    if (this.questionMetaData.editorState) {
                      this.editorState = this.questionMetaData.editorState;
                      this.sliderOptions =
                        this.questionMetaData.interactions.response1;
                      this.sliderDatas =
                        this.questionMetaData.interactions.response1;
                  ...
                  if (this.questionInteractionType === "choice") {
                    const responseDeclaration =
                      this.questionMetaData.responseDeclaration;
                    this.scoreMapping = _.get(
                      responseDeclaration,
                      "response1.mapping"
                    );
                    const templateId = this.questionMetaData.templateId;
                    const numberOfOptions =
                      this.questionMetaData?.editorState?.options?.length || 0;
                    const maximumOptions = _.get(
                      this.questionInput,
                      "config.maximumOptions"
                    );
                    this.editorService.optionsLength = numberOfOptions;
                    const options = _.map(
                      this.questionMetaData?.editorState?.options,
                      (option) => ({ body: option.value.body })
                    );
                    const question =
                      this.questionMetaData?.editorState?.question;
                  ...
                  if (_.has(this.questionMetaData, "primaryCategory")) {
                    this.editorState.primaryCategory = _.get(
                      this.questionMetaData,
                      "primaryCategory"
                    );
                  }
                  ...
                  if (!_.isEmpty(this.editorState.solutions)) {
                    this.selectedSolutionType =
                      this.editorState.solutions[0].type;
                    ...
                    if (this.selectedSolutionType === "video") {
                      const index = _.findIndex(
                        this.questionMetaData.media,
                        (o) => {
                          return (
                            o.type === "video" &&
                            o.id === this.editorState.solutions[0].value
                          );
                        }
                      );
                      this.videoSolutionName =
                        this.questionMetaData.media[index].name;
                      this.videoThumbnail =
                        this.questionMetaData.media[index].thumbnail;
                    }
                    if (this.selectedSolutionType === "html") {
                      this.editorState.solutions =
                        this.editorState.solutions[0].value;
                    }
                  }
                  ...
                  if (
                    !_.isUndefined(
                      this.editorService?.editorConfig?.config?.renderTaxonomy
                    )
                  ) {
                    this.subMenuConfig();
                  }
                  this.contentComment = _.get(
              ...
              (err: ServerResponse) => {
                const errInfo = {
                  errorMsg:
                    "Fetching question details failed. Please try again...",
                };
                return throwError(
        ...
        if (_.isUndefined(this.questionId)) {
          ...
          if (this.questionInteractionType === "default") {
            if (this.questionCategory) {
              editorState = _.get(
                this.configService,
                `editorConfig.defaultStates.nonInteractiveQuestions.${this.questionCategory}`
              );
            } else {
            ...
            this.editorState = new McqForm(
              { question: "", options: [] },
              {
                numberOfOptions: _.get(
                  this.questionInput,
                  "config.numberOfOptions"
                ),
                maximumOptions: _.get(
                  this.questionInput,
                  "config.maximumOptions"
                ),
              }
  ...
  saveContent() {
    ...
    if (
      ...
      this.toasterService.error(
        _.get(this.configService, "labelConfig.messages.error.044")
      );
    }
  }
  ...
  onConsentSubmit(event) {
    ...
    if (event) {
      this.questionMetaData = _.assign(this.questionMetaData, {
        isReviewModificationAllowed: event.editingConsent,
      });
      this.sendForReview();
    }
  }
  ...
  sendForReview() {
    if (!_.get(this.editorService.editorConfig, "config.skipTwoLevelReview")) {
      let callback = function () {
        this.editorService.reviewContent(this.questionId).subscribe(
          (data) => {
            this.toasterService.success(
              _.get(this.configService, "labelConfig.messages.success.002")
            );
            this.redirectToChapterList();
          },
          (err) => {
            this.toasterService.error(
              _.get(this.configService, "labelConfig.messages.error.002")
            );
          }
        );
      };
      ...
      const callback = this.addResourceToQuestionset.bind(
        this,
        publishCallback
      );
      this.upsertQuestion(callback);
  ...
  requestForChanges(comment) {
    this.editorService.submitRequestChanges(this.questionId, comment).subscribe(
      (res) => {
        this.toasterService.success(
          _.get(this.configService, "labelConfig.messages.success.003")
        );
        this.redirectToChapterList();
      },
      (err) => {
        this.toasterService.error(
          _.get(this.configService, "labelConfig.messages.error.003")
        );
      }
    );
  }
  ...
  sendQuestionForPublish(event) {
    this.editorService.publishContent(this.questionId, event).subscribe(
      (res) => {
        if (
          !(
            this.creationMode === "sourcingReview" &&
            this.editorService.isReviewModificationAllowed
          )
        ) {
          this.toasterService.success(
            _.get(this.configService, "labelConfig.messages.success.037")
          );
        }
        this.redirectToChapterList();
      },
      (err) => {
        this.toasterService.error(
          _.get(this.configService, "labelConfig.messages.error.038")
        );
      }
    );
  }
  ...
  rejectQuestion(comment) {
    ...
    if (
      this.creationMode === "orgreview" &&
      editableFields &&
      !_.isEmpty(editableFields[this.creationMode])
    ) {
      this.validateFormFields();
      if (this.showFormError === true) {
        this.toasterService.error(
          _.get(this.configService, "labelConfig.messages.error.029")
        );
        return false;
      }
  ...
  sourcingUpdate(event) {
    ...
    let successMessage = "";
    this.editorService
      .fetchCollectionHierarchy(this.questionSetId)
      .subscribe((res) => {
        const questionSet = res.result["questionSet"];
        switch (event.button) {
          case "sourcingApproveQuestion":
            questionIds = questionSet.acceptedContributions || [];
            successMessage = _.get(
              this.configService,
              "labelConfig.messages.success.038"
            );
            break;
          case "sourcingRejectQuestion":
            ...
            successMessage = _.get(
              this.configService,
              "labelConfig.messages.success.039"
            );
            break;
          default:
            break;
        ...
        event["requestBody"] = this.prepareSourcingUpdateBody(
          questionIds,
          comments
        );
        this.editorService
          .updateCollection(this.questionSetId, event)
          .subscribe((res) => {
  ...
  sendBackQuestion(event) {
    this.questionService.readQuestion(this.questionId, "status").subscribe(
      (res) => {
        const requestObj = {
          question: {
        ...
        this.questionService
          .updateQuestion(this.questionId, requestObj)
          .subscribe((res) => {
            this.toasterService.success(
              _.get(this.configService, "labelConfig.messages.success.040")
            );
            this.redirectToChapterList();
          });
      },
  ...
  validateQuestionData() {
    ...
    if (this.questionInteractionType === "choice") {
      ...
      if (
        _.get(this.editorState, "interactionTypes[0]") === "choice" &&
        ...
        this.toasterService.error(
          _.get(this.configService, "labelConfig.messages.error.005")
        );
        this.showFormError = true;
        return;
      } else {
      ...
      const optionValid = _.find(
        this.editorState.options,
        (option) =>
          option.body === undefined ||
          option.body === "" ||
          option.length > this.setCharacterLimit
      );
      if (
        optionValid ||
        (!this.editorState.answer &&
          this.sourcingSettings?.enforceCorrectAnswer)
      ) {
        this.showFormError = true;
        return;
      } else {
    ...
    this.editorService
      ...
      .subscribe(
        ...
        (err) => {
          const errInfo = {
            errorMsg:
              "Adding question to questionset failed. Please try again.",
          };
  ...
  videoDataOutput(event) {
    if (event) {
      ...
      videoMedia.baseUrl =
        _.get(this.editorService.editorConfig, "context.host") ||
  ...
  deleteSolution() {
    if (this.selectedSolutionType === "video") {
      this.mediaArr = _.filter(
        this.mediaArr,
        (item: any) => item.id !== this.editorState.solutions
      );
    }
  ...
  getQuestionMetadata() {
    ...
    if (this.questionInteractionType === "choice") {
      metadata.body = this.getMcqQuestionHtmlBody(
        this.editorState.question,
        this.editorState.templateId
      );
    } else {
      metadata.responseDeclaration = this.getResponseDeclaration(
        this.questionInteractionType
      );
    }

    if (
      !_.isUndefined(this.selectedSolutionType) &&
      !_.isEmpty(this.selectedSolutionType)
    ) {
      const solutionObj = this.getSolutionObj(
        this.solutionUUID,
        this.selectedSolutionType,
        this.editorState.solutions
      );
    ...
    if (_.get(this.creationContext, "objectType") === "question") {
      metadata.programId = _.get(
        this.editorService,
        "editorConfig.context.programId"
      );
      metadata.collectionId = _.get(
        this.editorService,
        "editorConfig.context.collectionIdentifier"
      );
      metadata.organisationId = _.get(
        this.editorService,
        "editorConfig.context.contributionOrgId"
      );
    }
    if (this.questionInteractionType === "choice") {
      metadata.responseDeclaration.response1.maxScore = this.maxScore;
      metadata.responseDeclaration.response1.correctResponse.outcomes.SCORE =
        this.maxScore;
    }
    metadata = _.merge(metadata, _.pickBy(this.childFormData, _.identity));
    if (_.get(this.creationContext, "objectType") === "question") {
      metadata.isReviewModificationAllowed = !!_.get(
        this.questionMetaData,
        "isReviewModificationAllowed"
      );
    }
  ...
  getMcqQuestionHtmlBody(question, templateId) {
    const mcqTemplateConfig = {
      // tslint:disable-next-line:max-line-length
      mcqBody:
        "<div class='question-body' tabindex='-1'><div class='mcq-title' tabindex='0'>{question}</div><div data-choice-interaction='response1' class='{templateClass}'></div></div>",
    };
    const { mcqBody } = mcqTemplateConfig;
    const questionBody = mcqBody
      .replace("{templateClass}", templateId)
      ...
      _.merge(
        {
          ...
          ..._.pick(_.get(this.editorService.editorConfig, "context"), [
            "board",
            "medium",
            "gradeLevel",
            "subject",
        ...
        {
          ..._.pick(
            this.questionSetHierarchy,
            this.configService.sessionContext
          ),
        }
      ),
      (key) => _.isEmpty(key)
  ...
  setQuestionTypeValues(metaData) {
    ...
    metaData.interactions.validation = {
      required: this.childFormData.markAsNotMandatory === "Yes" ? "No" : "Yes",
    };
    ...
    if (
      !_.isEmpty(this.sliderDatas) &&
      this.questionInteractionType === "slider"
    ) {
  ...
  prepareRequestBody() {
    ...
    return {
      ...
      hierarchy: this.editorService.getHierarchyObj(
        data,
        questionId,
        selectedUnitId
      ),
  ...
  prepareQuestionBody() {
    return this.questionId
      ? {
          question: _.omit(this.getQuestionMetadata(), [
            "mimeType",
            "creator",
            "createdBy",
            "organisationId",
  ...
  prepareSourcingUpdateBody(questionIds, comments?) {
    const sourcingUpdateAttribute =
      this.actionType === "sourcingApproveQuestion"
        ? "acceptedContributions"
        : "rejectedContributions";
    const collectionObjectType = _.replace(
      _.lowerCase(this.creationContext["collectionObjectType"]),
      " ",
      ""
    );
    const requestBody = {
    ...
    if (this.actionType === "sourcingRejectQuestion") {
      requestBody.request[collectionObjectType][
  ...
  upsertQuestion(callback) {
    ...
    this.questionService
      .upsertQuestion(this.questionId, requestBody)
      .pipe(
        finalize(() => {
      ...
      .subscribe(
        (response: ServerResponse) => {
          if (!_.includes(["submitQuestion"], this.actionType)) {
            this.toasterService.success(
              _.get(this.configService, "labelConfig.messages.success.013")
            );
          }
  ...
  saveQuestions(requestBody, type) {
    this.showHideSpinnerLoader(true);
    this.questionService
      .updateHierarchyQuestionCreate(requestBody)
      .pipe(
        finalize(() => {
          this.showHideSpinnerLoader(false);
        })
  ...
  saveUpdateQuestions() {
    ...
    this.showHideSpinnerLoader(true);
    this.questionService
      .updateHierarchyQuestionUpdate(requestBody)
      .pipe(
        finalize(() => {
          this.showHideSpinnerLoader(false);
        })
  ...
  setParentConfig(parentConfig) {
    this.questionSetHierarchy.showSolutions = !_.isUndefined(
      parentConfig?.showSolutions
    )
      ? parentConfig.showSolutions
      : "No";
    this.questionSetHierarchy.shuffle = !_.isUndefined(parentConfig?.shuffle)
      ? parentConfig.shuffle
      : true;
    this.questionSetHierarchy.showFeedback = !_.isUndefined(
      parentConfig?.showFeedback
    )
      ? parentConfig.showFeedback
      : "No";
  }
  ...
  setQumlPlayerData(questionId: string) {
    ...
    if (this.questionSetHierarchy.shuffle === true) {
      ...
      if (questionMetadata.qType === "SA") {
        this.questionSetHierarchy = _.omit(
          this.questionSetHierarchy,
          "maxScore"
        );
      ...
      if (!_.isUndefined(questionId)) {
        ...
        _.forEach(hierarchyChildren, (child) => {
          if (child.children) {
            ...
            questionTitle =
              `Q${(index + 1).toString()} | ` +
              (_.get(this.categoryLabel, `${question.primaryCategory}`) ||
                question.primaryCategory);
          } else {
            index = _.findIndex(
              hierarchyChildren,
              (node) => node.data.id === questionId
            );
            ...
            questionTitle =
              ...
              (_.get(
                this.categoryLabel,
                `${_.get(question, "data.primaryCategory")}`
              ) || _.get(question, "data.primaryCategory"));
          }
        });
      } else {
        ...
        if (!_.isUndefined(this.questionPrimaryCategory)) {
          questionTitle =
            questionTitle +
            (_.get(this.categoryLabel, `${this.questionPrimaryCategory}`) ||
  ...
  valueChanges(event) {
    if (_.has(event, "maxScore")) {
      ...
      event.maxScore = !_.isNull(event.maxScore)
        ? parseInt(event.maxScore)
        : this.maxScore;
  ...
  validateFormFields() {
    _.forEach(this.questionFormConfig, (formFieldCategory) => {
      if (
        formFieldCategory.required &&
        !this.childFormData[formFieldCategory.code]
      ) {
        this.showFormError = true;
        this.toasterService.error(
          _.get(this.configService, "labelConfig.messages.error.008")
        );
        return false;
      }
    });
  ...
  previewFormData(status) {
    ...
    _.forEach(formConfig, (formFieldCategory) => {
      if (
        _.has(formFieldCategory, "editable") &&
        !_.isUndefined(formFieldCategory.editable)
      ) {
        formFieldCategory.editable = status
          ? _.find(this.leafFormConfig, { code: formFieldCategory.code })
              .editable
          : status;
  ...
  populateFormData() {
    ...
    _.forEach(this.leafFormConfig, (formFieldCategory) => {
      if (!_.isUndefined(this.questionId)) {
        if (
          formFieldCategory.code === "maxScore" &&
          this.questionInteractionType === "choice"
        ) {
          this.childFormData[formFieldCategory.code] = _.has(
            this.questionMetaData,
            "responseDeclaration.response1.maxScore"
          )
            ? _.get(
                this.questionMetaData,
                "responseDeclaration.response1.maxScore"
          ...
          formFieldCategory.code === "allowMultiSelect" &&
          this.questionInteractionType === "choice"
        ) {
          this.childFormData[formFieldCategory.code] =
            _.get(
          ...
          this.questionMetaData &&
          _.has(this.questionMetaData, formFieldCategory.code)
        ) {
          formFieldCategory.default =
            this.questionMetaData[formFieldCategory.code];
          this.childFormData[formFieldCategory.code] =
            this.questionMetaData[formFieldCategory.code];
        }
        try {
          ...
          if (
            this.questionMetaData &&
            _.has(availableAlias, formFieldCategory.code)
          ) {
            let defaultValue = _.get(
              this.questionMetaData,
        ...
        // tslint:disable-next-line:max-line-length
        const questionSetDefaultValue = _.get(
          this.questionSetHierarchy,
          formFieldCategory.code
        )
          ? _.get(this.questionSetHierarchy, formFieldCategory.code)
          : "";
        const defaultEditStatus =
          _.find(this.initialLeafFormConfig, { code: formFieldCategory.code })
            .editable === true;
        formFieldCategory.default = defaultEditStatus
          ? ""
          : questionSetDefaultValue;
  ...
  get dependentQuestions() {
    try {
      return this.subMenus.filter(
        (menu) => menu.id === "addDependantQuestion"
      )[0].value;
    } catch (error) {
      return null;
    }
  ...
  subMenuConfig() {
    this.subMenus = [
      ...
      },
      {
        ...
        label: "Tip",
        enabled: _.get(this.questionMetaData, "instructions.en[0]")
          ? true
          : false,
        type: "input",
      ...
      },
      {
        ...
        type: "",
        show:
          _.get(this.sourcingSettings, "showAddSecondaryQuestion") &&
          !this.questionInput.setChildQueston,
      },
    ];
    if (
      !_.get(this.sourcingSettings, "showAddSecondaryQuestion") &&
      ...
      this.showOptions =
        this.questionInput.setChildQueston === true ? true : false;
    }
  }
  ngOnDestroy() {
  ...
  buildCondition(type) {
    ...
    const questionId = this.questionId ? this.questionId : UUID.UUID();
    const data = this.treeNodeData;
    const hierarchyData = this.editorService.getHierarchyObj(
      data,
      "",
  ...
  updateTarget(questionId) {
    if (
      !_.isEmpty(this.branchingLogic) &&
      _.get(
        this.branchingLogic,
        `${this.editorService.parentIdentifier}.target`
      )
    ) {
      if (
        this.branchingLogic[
        ...
        return [
          ...this.branchingLogic[this.editorService.parentIdentifier].target,
        ];
      }
      return [
        ...this.branchingLogic[this.editorService.parentIdentifier].target,
  ...
  getParentQuestionOptions(questionId) {
    ...
    this.questionService.readQuestion(questionId).subscribe((res) => {
  ...
  setCondition(data) {
    const Condition = _.get(
      data?.branchingLogic,
      `${this.questionId}.preCondition.and[0]`
    );
    ...
    _.forEach(children, (data) => {
      if (data.identifier === this.selectedSectionId) {
        ...
        if (_.get(data?.branchingLogic, `${this.questionId}.source[0]`)) {
          ...
          this.getParentQuestionOptions(
            data.branchingLogic[this.questionId].source[0]
          );
  ...
  onQualityFormSubmit(event) {
    switch (event.action) {
      case "submit":
        this.saveQualityParameters(
          event.data,
          this.sendQuestionForPublish.bind(this, {})
        );
        break;
      case "requestChange":
        ...
        this.saveQualityParameters(
          event.data,
          this.openRequestChangesPopup.bind(this, {})
        );
        break;
      default:
  ...
  calculateMinMaxScore(metaData) {
    ...
    if (!_.isEmpty(metaData.responseDeclaration.response1.mapping)) {
      metaData.responseDeclaration.response1.mapping.forEach((item) =>
        scores.push(item.outcomes.score)
      );
      metaData.responseDeclaration.response1.minScore = Math.min(...scores);
      if (
        _.get(metaData, "responseDeclaration.response1.cardinality") ===
        "multiple"
      ) {
        let sum = 0;
        metaData.responseDeclaration.response1.mapping.forEach(
          (item) => (sum += +item.outcomes.score)
        );
